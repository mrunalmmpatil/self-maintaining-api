"""Behavioural tests pinned to the Inventory API v2 contract.
...
class CatalogListingTest(unittest.TestCase):
    ...
    def test_listing_is_fetched_from_the_v2_catalog_path(self):
        with mock.patch.object(client, "_http_get", return_value=dict(V2_LISTING)) as get:
            pricing.catalog_names("peripherals")
            self.assertTrue(get.call_args[0][0].startswith("/catalog/products?"))

    def test_listing_still_sends_the_category_parameter(self):
        # category is unchanged in v2; the listing must keep passing it through.
        with mock.patch.object(client, "_http_get", return_value=dict(V2_LISTING)) as get:
            pricing.catalog_names("peripherals")
            self.assertIn("category=peripherals", get.call_args[0][0])
